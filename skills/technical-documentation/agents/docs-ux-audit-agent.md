...
Goal:
- audit every file in the shard against `references/large-docs-audit.md` section 5 and return findings a remediation agent can act on without re-reading your reasoning
...
- for pages over 20k characters, add a split plan per the oversized page protocol (section 7).
...
- findings as ledger JSON objects (`references/large-docs-audit.md` section 9) with severity and confidence
