...
3. Preserve every anchor. List the old `page#anchor` targets and their new destinations, then keep each one alive **on the parent page** as an authored `<a id>` stub linking to the child. Redirects match on pathname and cannot carry a fragment, so a `/page/anchor` route does not rescue a `/page#anchor` link. Compute the ids with the repo's publishing parser, not a hand-rolled slug. The link checker cannot detect this defect, because a split rewrites the repo's own links while external links stay broken — assert the old ids resolve instead of trusting a clean audit. See "A path is not a fragment" in `references/pr-program.md`.
