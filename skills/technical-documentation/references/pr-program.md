...
4. Treat repeated failures on an unrelated-looking check as a signal, not as flakiness. Read the failing test name on every run. A shard that fails four times is telling you something, and the test that fails may change between runs.
5. When a merged PR breaks the default branch, fix forward immediately with a PR that names the invariant it violated and quotes the assertion.
