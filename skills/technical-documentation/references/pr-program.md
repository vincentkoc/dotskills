...
1. **A redirect cannot carry a fragment.** Redirect rules match on pathname. A docs generator will typically reject a source containing `#` outright. So `/page/anchor` as a new route does nothing for anyone holding `/page#anchor` — a path and a fragment are different addresses. Read the redirect resolver before designing the strategy.
...
For a genuine refusal, there is no fix inside the PR that keeps its central promise. Editing those lines breaks the byte-identity the split is built on. What works is evidence and escalation: cite the identical lines at their blob on the default branch, entropy-scan the added lines and report the maximum so "no real secret" is measured rather than asserted, and state what the alternative shape would cost — keeping the affected section on the index leaves it as unchanged context, at the price of not splitting it.
